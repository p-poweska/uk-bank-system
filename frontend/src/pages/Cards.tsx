...
const Cards = () => {
  ...
  const refreshData = async () => {
    try {
      ...
      if (selectedAccount) {
        const updatedAccount = fetchedAccounts.find(
          (account: any) => account.id === selectedAccount.id,
        );
  ...
  const handleSyncStatus = async () => {
    ...
    try {
      await api.post('/cards/sync-status/', {
        local_card_id: activeCardId,
      });

      await refreshData();
    } catch (error: any) {
      console.error('Card synchronization failed:', error);

      alert(
        error.response?.data?.error ||
          'Could not synchronize the card status.',
      );
    }
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="text-left flex-1 overflow-hidden flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded shrink-0 border ${
                        isJunior
