...
class UKPSRegistration(models.Model):
    ...
    # BACS only: last observed settled liquidity, used to detect inbound credits
    # (the cycle.settled event carries no detail, so we credit the balance delta).
    bacs_settled_balance = models.DecimalField(
